...
def plot_vehicle(image, centre, dimension, v_color, text, t_color, circle = False):
    ...
    if circle == True:
        ...
        image = cv2.putText(image, text, bot_left, cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color = t_color, thickness=1 )
    return image